...
const Sidebar = ({
  ...
  return (
    <>
      ...
      <aside
        className={`fixed left-0 top-0 z-50 h-screen bg-white/80 text-slate-700 shadow-sm backdrop-blur-xl transition-all duration-300 dark:bg-slate-950/80 dark:text-slate-100 lg:fixed lg:shadow-none ${
          isMobileOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"
        } ${isCollapsed ? "w-20" : "w-64"}`}
      >
        <div className="flex h-16 items-center border-b border-slate-200 px-4 dark:border-white/10">
          <div className="flex items-center gap-3 overflow-hidden">
            <div
              className={`inline-flex h-10 w-10 items-center justify-center rounded-2xl text-white shadow-lg ${accentClassName}`}
            >
        ...
        <button
          type="button"
          className="absolute -right-4 top-1/2 z-20 hidden h-9 w-9 -translate-y-1/2 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-600 shadow-lg transition hover:bg-slate-50 hover:text-slate-900 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 dark:hover:bg-slate-800 lg:inline-flex"
          ...
          {items.map((item) => {
            ...
            return (
              <button
                ...
                className={`group flex w-full items-center rounded-xl px-3 py-2.5 text-sm font-semibold transition-all duration-300 ${
                  isActive
                    ? `${accentButtonClassName} text-white shadow-sm ring-1 ring-white/20`
